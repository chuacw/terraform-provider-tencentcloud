...
func dataSourceTencentCloudAutoScalingGroups() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTencentCloudAutoScalingGroupsRead,
...
func dataSourceTencentCloudAutoScalingGroupsRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] tencentcloud_autoscaling_groups - param: %v", params)
	...
	if jsonresp.Response.Error.Code != "" {
		return fmt.Errorf(
			"tencentcloud_autoscaling_groups got error, code:%v, message:%v",
	...
	if len(zoneList) == 0 {
		return errors.New("No autoscaling groups found")
	}
	...
	for _, zone := range zoneList {
		log.Printf(
			"[DEBUG] tencentcloud_autoscaling_groups - Group found id: %v, name:% v, description: %v, state: %v",
			zone.ZoneId,
			zone.Zone,
			zone.ZoneName,
			zone.ZoneState,
	...
	d.SetId(id)
	log.Printf("[DEBUG] tencentcloud_autoscaling_groups - instances[0]: %#v", result[0])
